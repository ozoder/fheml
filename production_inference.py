...
class ProductionFHEInference:
    ...
    def __init__(self, model: FHEMLPClassifier, context: Optional[ts.Context] = None):
        self.model = model
    ...
    def predict(
        ...
        if isinstance(input_data, ts.CKKSTensor):
            ...
            raise TypeError(f"Input must be torch.Tensor or ts.CKKSTensor, got {type(input_data)}")
    ...
    def predict_batch(
        ...
        iterator = tqdm(inputs, desc="Batch inference") if show_progress else inputs
    ...
    def evaluate_accuracy(
        ...
        predictions = self.predict_batch(inputs, return_confidence=False, show_progress=show_progress)
    ...
    def get_performance_stats(
        ...
        predictions_with_confidence = self.predict_batch(inputs, return_confidence=True)
        predictions = [pred for pred, _ in predictions_with_confidence]
        confidences = [conf for _, conf in predictions_with_confidence]
        
        # Basic accuracy
        accuracy = sum(1 for pred, true in zip(predictions, true_labels) 
                      if pred == true) / len(predictions)
        ...
        for class_id in range(10):
            class_preds = [pred for pred, true in zip(predictions, true_labels) if true == class_id]
            class_labels = [true for true in true_labels if true == class_id]
            if len(class_labels) > 0:
                class_acc = sum(1 for pred, true in zip(class_preds, class_labels) 
                               if pred == true) / len(class_labels)
                per_class_accuracy[class_id] = class_acc
        
        # Input type breakdown
        encrypted_count = sum(1 for inp in inputs if isinstance(inp, ts.CKKSTensor))
...
class SecureInferenceServer:
    ...
    def load_model(self, model_path: str):
        ...
        self.model = FHEMLPClassifier(
            ...
            use_polynomial_activation=model_config.get("use_polynomial_activation", False)
        )
        
        self.model.set_parameters(checkpoint["model_state"])
        self.inference_engine = ProductionFHEInference(self.model, self.context)
    ...
    def process_request(
        ...
        if isinstance(input_data, torch.Tensor) and not self.allow_plain_input:
            raise ValueError("This server only accepts encrypted inputs for security.")
        
        # Process the request
        start_time = torch.cuda.Event(enable_timing=True) if torch.cuda.is_available() else None
        
        try:
            result = self.inference_engine.predict(input_data, request_confidence)
            ...
            if request_confidence:
                ...
                response = {
                    ...
                    "input_type": "encrypted" if isinstance(input_data, ts.CKKSTensor) else "plain",
                    "status": "success"
                }
            else:
                response = {
    ...
    def batch_process(
        ...
        for request in batch_requests:
            ...
            response = self.process_request(input_data, request_confidence, client_id)
    ...
    def get_server_stats(self) -> dict:
        ...
        return {
            ...
            "model_architecture": {
                "input_dim": self.model.layers[0].in_features if self.model else None,
                "hidden_dims": [layer.out_features for layer in self.model.layers[:-1]] if self.model else None,
                "num_classes": self.model.layers[-1].out_features if self.model else None
